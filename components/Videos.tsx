...
const videos = [
  {
    src: '/videos/menton.mp4',
    title: 'Proyección de Mentón',
    tag: 'Rellenos',
    duration: '1 min',
  },
  {
    src: '/videos/bioestimulador.mp4',
    title: 'Bioestimulador Radiesse',
    tag: 'Bioestimulación',
    duration: '2 min',
  },
  {
    src: '/videos/bruxismo.mp4',
    title: 'Toxina para Bruxismo',
    tag: 'Neuromoduladores',
    duration: '3 min',
  },
  {
    src: '/videos/procedimientos.mp4',
    title: 'Procedimientos Variados',
    tag: 'General',
    duration: '2 min',
  },
]
  ...
  return (
    ...
    >
      <video
        ...
        style={{ filter: 'sepia(14%) saturate(88%) brightness(0.78)' }}
      />
      ...
      <div className={`absolute inset-0 flex items-center justify-center z-20 transition-opacity duration-300 ${playing ? 'opacity-0' : 'opacity-100'}`}>
        <div className="w-12 h-12 border border-ivory/40 rounded-full flex items-center justify-center backdrop-blur-sm bg-charcoal/20 group-hover:border-gold group-hover:bg-gold/10 transition-all duration-300">
          <svg className="w-4 h-4 text-ivory fill-current ml-0.5" viewBox="0 0 24 24">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 z-20">
        <p
          className="font-playfair text-ivory font-normal mb-1"
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            ...
            <p className="font-inter text-ink-muted text-sm max-w-xs text-right hidden md:block leading-relaxed">
              Pasa el cursor sobre cada video para ver el procedimiento en acción.
            </p>
          </div>
        </motion.div>

        {/* Video grid — vertical videos in a horizontal row */}
        ...
        {/* CTA */}
        <motion.div
          className="mt-12 flex flex-col md:flex-row items-center justify-between gap-6 pt-8 border-t border-stone"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ delay: 0.2, duration: 0.8 }}
        >
          <p className="font-cormorant italic text-ink-muted text-xl">
            "Cada detalle importa. Cada procedimiento, perfecto."
          </p>
          <a
            href="https://wa.me/573174538636"
            target="_blank"
            rel="noopener noreferrer"
            className="flex-shrink-0 px-7 py-3.5 border border-gold/60 text-gold font-inter text-[10px] tracking-[0.25em] uppercase hover:bg-gold hover:text-charcoal transition-all duration-300"
          >
            Agendar mi Consulta
          </a>
        </motion.div>
